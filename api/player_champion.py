...
class PlayerChampion:
    ...
    def set_active_player_data(self) -> None:
        ...
        if is_process_running(self.process_name):
            ...
            #     if data and "summonerName" in data:
            #         self.summoner_name = data["summonerName"].split("#")[0]  # it includes tag
            #         self.current_gold = data.get("currentGold", 0)
            #         stats = data.get("championStats", {})
            #         self.max_hp = stats.get("maxHealth", 0)
            #         self.current_hp = stats.get("currentHealth", 0)
            #     else:
            #         self.logger.debug(f"activeplayer data missing summonerName: {data}")
            # except Exception as e:
            #     self.logger.warning(f"Error setting active player data: {e}")
            data = self._request_api.get(url="/liveclientdata/activeplayer").json()
            self.summoner_name = data["summonerName"].split("#")[0]  # it includes tag
    ...
    def set_game_events_data(self) -> None:
        ...
        if is_process_running(self.process_name):
            ...
            # except Exception as e:
            #      self.logger.warning(f"Error setting game events data: {e}")
            self.logger.debug("Setting game state")
            events = self._request_api.get(url="/liveclientdata/eventdata").json()
    ...
    def get_game_time(self) -> float:
        ...
        if is_process_running(self.process_name):
            try:
                data = self._request_api.get(url="/liveclientdata/gamestats").json()
    ...
    def set_player_state(self) -> None:
        ...
        if is_process_running(self.process_name):
            ...
            #         # Debug: Print expected name vs found names
            #         print(f"DEBUG: Looking for match for '{self.summoner_name}' in playerlist...")
            #         
            #         found = False
            #         for player in player_list:
            #             # Compare stripped names to handle Riot IDs
            #             p_name_stripped = player["summonerName"].split("#")[0]
            #             # Try matching either exact or stripped
            #             if player["summonerName"] == self.summoner_name or p_name_stripped == self.summoner_name:
            #                 print(f"DEBUG: Match found! Team: {player['team']}, Dead: {player['isDead']}")
            #                 self.side = player["team"]
            #                 self.is_alive = not player["isDead"]
            #                 found = True
            #                 # Only sleep if actually dead, to avoid stalling the loop unnecessarily
            #                 if player["isDead"]:
            #                      sleep(player["respawnTimer"])
            #                 break
            #         
            #         if not found:
            #             print(f"DEBUG: No match found for {self.summoner_name}. Available: {[p.get('summonerName', 'Unknown') for p in player_list]}")
            #     else:
            #         self.logger.debug(f"playerlist returned non-list (likely error): {player_list}")
            # except Exception as e:
            #     self.logger.warning(f"Error setting player state: {e}")
            player_list = self._request_api.get("/liveclientdata/playerlist").json()
            for player in player_list:
    ...
    def get_current_items(self) -> List[str]:
        """
        Get list of current items on champion by filtering playerlist
        :return: current items
        """
        if self.side and self.game_in_progress:
            # try:
            #     # The direct endpoint with param seems to be failing, so let's get the full list and filter in python
            #     response = self._request_api.get("/liveclientdata/playerlist").json()
            #     
            #     if isinstance(response, list):
            #         for player in response:
            #             # Handle Riot ID matching
            #             p_name = player["summonerName"]
            #             p_name_stripped = p_name.split("#")[0]
            #             
            #             if p_name == self.summoner_name or p_name_stripped == self.summoner_name:
            #                 items = player.get("items", [])
            #                 current_items = [item["displayName"] for item in items]
            #                 
            ...
            #         
            #         print(f"DEBUG: Player {self.summoner_name} not found in playerlist for items.")
            #         return []
            #     else:
            #         return []
            # except Exception as e:
            #     print(f"DEBUG: Error getting items: {e}")
            #     return []
    ...
    def get_player_abilities(self) -> Dict:
        ...
        if self.side and self.game_in_progress:
            # try:
            #     data = self._request_api.get(url="/liveclientdata/activeplayerabilities").json()
            #     if isinstance(data, dict):
            #         return data
            # except Exception as e:
            #     self.logger.warning(f"Error getting player abilities: {e}")
    ...
    def buy_items(self, item_path: Tuple[Items]) -> True:
        ...
        # Check if we have already bought all items in the path
        # if self.item >= len(item_path):
        #     return False
